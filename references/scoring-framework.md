...
## 全球基准百分位（Percentile Ranking）

为每个维度和总分计算百分位排名，让用户理解自己的相对位置。

### 基准分布模型

基于 DAMC Beta 用户群体，各维度近似正态分布：

| 维度 | 均值 (μ) | 标准差 (σ) | 说明 |
|------|---------|-----------|------|
| D (蒸馏价值) | 52 | 18 | 多数人有一定领域知识但未结构化 |
| A (抗蒸馏) | 55 | 15 | 人际/创造力分布较集中 |
| M (AI驾驭) | 40 | 22 | 两极分化严重：高手很高，新手很低 |
| C (职业适配) | 50 | 16 | 综合维度，分布较均匀 |
| Overall | 50 | 15 | 加权综合 |
...
在 `window.DAMC_DATA` 中添加 `percentiles` 字段：
